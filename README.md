...
## 부록: ShadCN + Tailwind CSS(v4) 설치 및 적용 절차 (Vite + React 기준)
...
### 3. Tailwind CSS import (src/index.css)
```css
@import "tailwindcss";
```

### 4. Tailwind 설정 파일 (tailwind.config.js)
```js
module.exports = {
  content: [
    "./src/**/*.{js,jsx,ts,tsx}",
...
### 5. (중요) postcss.config.js/cjs 파일은 **생성하지 않음**
- Tailwind v4 + Vite 공식 방식에서는 postcss 설정 파일이 필요 없습니다.
- 기존에 있다면 삭제하세요.

### 6. index.css가 main.tsx(또는 index.tsx)에서 import되어 있는지 확인
```ts
import './index.css';
```
