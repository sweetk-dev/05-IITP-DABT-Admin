...
관리자 계정 초기 비밀번호 설정을 위한 해싱 테스트 도구를 제공합니다.
...
VALUES ('admin@example.com', '관리자', '시스템관리자', '$2a$10$...', true, NOW(), NOW());
...
💡 관리자가 최초 로그인 후 반드시 비밀번호를 변경하도록 안내하세요.
```

### 주의사항
- 이 도구는 개발/테스트 환경에서만 사용하세요.
- 생성된 SQL을 데이터베이스에 실행하여 관리자 계정을 생성할 수 있습니다.
- 관리자가 최초 로그인 후 반드시 비밀번호를 변경하도록 안내하세요.
