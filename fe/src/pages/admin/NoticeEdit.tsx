...
export default function AdminNoticeEdit() {
  ...
  const { data } = useDataFetching({ fetchFunction: () => getAdminNoticeDetail(noticeId), dependencies: [noticeId], autoFetch: !!noticeId });
  ...
  const handleSave = async () => {
    ...
    if (titleBytes > TITLE_MAX || contentBytes > CONTENT_MAX) { setError('입력 용량을 초과했습니다.'); return; }
    setLoading(true); setError(null);
    const res = await updateAdminNotice(noticeId, { title, content } as any);
    ...
    <Box id="admin-notice-edit-page" sx={{ p: SPACING.LARGE }}>
      ...
      <ThemedCard>
        <CardContent>
          ...
          <TextField id="notice-title" fullWidth label="제목" value={title} onChange={(e)=>setTitle(e.target.value)} sx={{ mb: 0.5 }} />
          <ByteLimitHelper id="notice-title-bytes" currentBytes={titleBytes} maxBytes={TITLE_MAX} />
          <TextField id="notice-content" fullWidth label="내용" value={content} onChange={(e)=>setContent(e.target.value)} multiline minRows={12} sx={{ mb: 0.5 }} />
